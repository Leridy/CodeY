...
export function GridContainer({ gridConfig, children, onLayoutChange, className }: GridContainerProps) {
  ...
  useEffect(() => {
    onLayoutChange?.(responsiveConfig);
  }, [responsiveConfig, onLayoutChange]);
  ...
  const renderChildren = useCallback(() => {
    return React.Children.map(children, (child) => {
      ...
      const panelId = (child.props as Record<string, unknown>).panelId as string | undefined;
  ...
  return (
    ...
    >
      <AnimatePresence mode="popLayout">{renderChildren()}</AnimatePresence>
    </div>
  );
}
